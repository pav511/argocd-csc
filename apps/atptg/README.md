...
| secretPermFixer | object | `{}` | This section sets the optional use of an init container for fixing permissions on secret files. If this section is used, each object listed can have the necessary attributes specified: _name_ (The label used for the init container) _containerPath_ (The path in the container where the secret files will be stored) _specialInstructions_ (This allows for optional instructions to be used when fixing permissions) |
...
| useInitContainer | bool | `false` | This sets whether the pod spec needs to use an init container. |
